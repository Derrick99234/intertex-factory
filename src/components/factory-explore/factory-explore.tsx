  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentText((prev) => {
        ...
        do {
          next =
            animatedTexts[Math.floor(Math.random() * animatedTexts.length)];
        } while (next === prev);
        return next;
      });
    }, 2000);
    ...
    <section className="flex flex-col items-center justify-center min-h-[70vh] bg-[#FAFAFA] py-8 sm:py-12 px-2 sm:px-4 md:px-0">
      ...
      <div className="flex flex-col md:flex-row items-center justify-center w-full ">
        <div className=" flex flex-col justify-center items-start w-full md:w-[613px] max-w-xs sm:max-w-md md:max-w-none md:mr-12 ">
          <div className="border-2 border-dashed border-secondary rounded-lg w-full md:h-[275px] h-[120px] sm:h-[120px] mb-4 sm:mb-8 flex flex-col justify-center items-start px-4 sm:px-8">
            <span className="block text-xl sm:text-3xl md:text-5xl font-bold text-secondary mb-2 sm:mb-8 transition-all duration-500">
              {currentText}
            </span>
          ...
          <div className="mb-2 mt-6 sm:mt-12">
            <span className="block text-base sm:text-lg text-[#152F24] mb-1">
              New fashion brand?
            </span>
            <span className="block text-base sm:text-lg text-[#152F24]">
              <span className="font-bold">Intertex</span> is here to be your
              first and last stop for all clothing needs.
            </span>
          </div>
          <Link
            href={"/enquiries/wholesale-enquiries"}
            className="bg-secondary flex justify-center items-center text-white text-sm sm:text-[19px] font-bold px-6 sm:px-10 py-2 sm:py-3 mt-2 shadow hover:bg-[#0d1333] transition-colors w-[230px] sm:w-[160px] md:w-[234px] h-[40px] sm:h-[48px] md:h-[64px]"
          >
            Get Started
          </Link>
        </div>
